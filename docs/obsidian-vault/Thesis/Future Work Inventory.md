...
| 8 | RAG-based coaching suggestions for lab assistants — 3-layer pipeline (SQL pre-filter → ChromaDB semantic search → LLM-generated hints) | Integrations | RAG Architecture, Full Roadmap Phase 9 | Designed (full architecture documented, dependencies not in requirements.txt) | Not mentioned — Should be in Ch6 |
...
### 4. RAG-based coaching suggestions (item #8)
A three-layer retrieval-augmented generation pipeline was designed (SQL pre-filter by student ID, ChromaDB semantic search over historical feedback, LLM-generated coaching hints) but not implemented. This represents the most architecturally novel future direction and would shift the system from monitoring to active support.
