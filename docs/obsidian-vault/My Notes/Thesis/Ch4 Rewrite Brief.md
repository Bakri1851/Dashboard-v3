...
- The backend reuses [`code/learning_dashboard/`](../../../../code/learning_dashboard/) directly (physical copy in `code2/learning_dashboard/`, with one minor refactor for the OpenAI-key guard so the FastAPI startup probe works without secrets). No analytics code was rewritten for V2.
