...
The contribution is the systematic coverage and the willingness to land negative findings as findings.
...
\paragraph{Joint weight and cutpoint optimisation via ordinal logistic regression.}
...
% FLAG: neither `mord` nor `statsmodels.OrderedModel` is in references.bib;
% decide whether to import (Phase-2 candidate) or drop the package names from the sentence below.
...
% FLAG: the 5.8% On Track figure is from the v2 Report Change List brief and is not
% in the shipped results-and-evaluation.tex; verify against data/eval/results.md or
% the rater summary, or restate qualitatively as "very few On Track snapshots".
The validation cohort produced no questions in the rater's \emph{Easy} band and only $5.8\%$ of snapshots in the \emph{On Track} band; the cohort distributions reported in Section~\ref{sec:eval-results} reflect a cohort skewed toward higher difficulty and lower performance.
...
Following directly from the cohort caveat in Section~\ref{sec:eval-thresholds}, the threshold-training pass should be re-run per cohort rather than the existing tuples reused indefinitely.
This is a deployment-side machine-learning maintenance concern rather than a model-design change: the underlying composite score is rank-portable across cohorts, but the cutpoints that turn it into a four-band label are not, and the maintenance loop should recognise that.
...
The OpenAI incorrectness scorer fell back to a midpoint score on the majority of the production submissions, which propagates into the composite as noise on the strongest single signal in the weight vector.
