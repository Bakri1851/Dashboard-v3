...
> **Sync note (2026-05-27 evening — toggles removed; v2 is the deployed default):** Any text below describing the "Optimised Weights (v2)" Settings card, the four `*_version` toggles, or v1 as the deployed default is **historical**. Those runtime toggles were removed; the trained v2 weights + Optuna hyperparameters + band thresholds are now the **unconditional deployed default**, and v1 hand-set weights survive only as `config.py` constants for the offline evaluation baseline. The cross-chapter framing also shifted: the training methodology migrates into Ch3/Ch4 (it's how the system works, not an evaluation experiment) and the headline §5.4 comparison is now OLS-vs-alternative-model-classes (the bake-off), with v1 demoted to a light baseline. See [[v2 Threshold Training Handoff]] §14.
