...
- **Figures**: pruned to 6 essentials (from 11 candidates), copied to [`Report/figures/evaluation/`](../../../../Report/figures/evaluation/) on 2026-05-25; inventory in the drafting plan
...
Two positive findings + two honest negative findings + one tie. The symmetry is the §5.4 contribution.

**Ch5 ToC restructure required (user to apply manually — see drafting plan):**

- §5.1.3 gains `\label{sec:eval-retro}`
- §5.1.5 NEW — "V2 Training Methodology" (methodology summary)
- §5.4.2 renamed from "Model Comparison - Struggle" → "Inter-rater Agreement (Cohen's κ)" (label preserved)
- §5.4.3 renamed from "Model Comparison - Difficulty" → "Model Comparison: Struggle (v1 vs v2 weights)" (label preserved)
- §5.4.9 NEW — "Honest Negative Findings" with `\label{sec:eval-v2-negative}`
- §5.4.10 NEW — "Hyperparameter Optimisation" with `\label{sec:eval-v2-hyperparams}`
...
5.1 Evaluation Design           [5 subsubs] — 4 DRAFTED + 1 DRAFTING-PLAN READY
    5.1.1 Scope of Evaluation
    5.1.2 Evidence Sources
    5.1.3 Retrospective Evaluation Protocol  [needs \label{sec:eval-retro}]
    5.1.4 Limitations of Evaluation
    5.1.5 V2 Training Methodology            [NEW — drafting plan Insertion 7]
...
5.4 Results                     [10 subsubs after restructure] — v2 BLOCKS DRAFTING-PLAN READY
    5.4.1 Baseline System Outputs              [drafting plan: cohort + 54%-Needs-Help finding]
    5.4.2 Inter-rater Agreement (Cohen's κ)    [RENAMED from "Model Comparison - Struggle"; drafting plan ready]
    5.4.3 Model Comparison: Struggle (v1 vs v2 weights)  [RENAMED from "Model Comparison - Difficulty"; drafting plan ready]
    ...
    5.4.9 Honest Negative Findings             [NEW; \label{sec:eval-v2-negative}; drafting plan]
    5.4.10 Hyperparameter Optimisation         [NEW; \label{sec:eval-v2-hyperparams}; drafting plan]
