...
> **Sync note (2026-05-27 evening — weight/hyperparam toggles removed):** The four `*_version` runtime toggles and the "Optimised Weights (v2)" Settings card described below (especially the §"Toggle → component" table and the Phase-5 wiring task list) are **historical**. Those toggles were removed from the V2 dashboard; the trained v2 weights + Optuna hyperparameters are now the **unconditional deployed default**, and v1 hand-set weights survive only as `config.py` constants for the offline evaluation baseline. The line-235 difficulty "NEGATIVE FINDING (AUC = 0.345)" is doubly superseded — by the target-swap note above (binary-AUC was the wrong framing) and the relabel note (difficulty is now ρ **+0.468**, a positive finding). The `struggle_model` / `difficulty_model` model-class selectors remain live. See [[v2 Threshold Training Handoff]] §14.
