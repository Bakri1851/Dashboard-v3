...
| 11  | Stage 4 Ch5 evaluation              | 🟩 mostly done | §5.1-§5.6 prose all in (2026-05-28); see "2026-05-28 — Ch5 evaluation chapter complete" entry below. Outstanding: Appendix B Settings-view re-shoot; Appendix E formulae (optional). Stale `% TODO:` comments swept. |
...
## 2026-05-28 — Ch5 evaluation chapter complete

After the [[v2 Threshold Training Handoff]] and the model-class bake-off session, the evaluation chapter is now substantially complete. See [[v2 Report Change List]] § "2026-05-28 — Ch5 session-complete sync" for the full per-section log; summary below.

- **§5.4 Quantitative Results — DONE.** Cohort characterisation (2×2 `cohort_distributions.png`, struggle top row, deployed v2 + recalibrated thresholds: 31/14/15/40% struggle, 7/4/22/67% difficulty); §5.4.2 Struggle Model Outputs (v2-only weight chart + heatmap + OLS diagnostic + per-fold ρ); §5.4.3 Model Comparison (`model_class_bakeoff.png` uniform single-colour + `framing_regression_vs_classification.png` cleaned + scatter + top-10 table + OLS-vs-classifier prose); §5.4.4 Hyperopt (prose-vs-table numeric reconciliation: K best 1→0, Δρ K +0.013→+0.009, Δρ τ +0.200→+0.160; "v1 default"/"v2 best" → "initial hand-set default"/"tuned best" in prose and table header); §5.4.5 NEW Band-threshold optimisation (motivation + method + 6-row constrained-CV κ `tab:eval-thresholds` + Δκ≥+0.05 promotion rule + deployed-cutpoint recalibration paragraph + cohort caveat); Ch4 `\label{sec:thresholds-struggle}` and `\label{sec:thresholds-difficulty}` anchors added at `implementation.tex` L664 / L721.
- **§5.5 Survey — DONE.** Three new figures replacing the original two: `eval-survey-baseline.png` (Q2 + Q3 / §5.5.2), `eval-survey-interpretability-trust.png` (Q5 + Q6 matrix + Q7, two-panel / §5.5.3), `eval-survey-comfort.png` (Q9 / §5.5.4); restructured `notebooks/survey_main.ipynb`; old `eval-survey-likert.png` + `eval-survey-clarity-intuitiveness.png` deleted. Prose for §5.5.1 (instrument + n=10 demographics), §5.5.2 (Q2 + Q3 + Q4 themes), §5.5.3 (Q5 + Q6 + Q7), §5.5.4 (Q9 + Q8 + Q10), §5.5.5 (Q11 + triangulation back to §5.4 / §5.6). NEW `tab:eval-survey-themes` at end of §5.5.5 (Q4/Q8/Q10/Q11 keyword themes).
- **§5.6 Discussion — DONE.** §5.6.1 IRT anti-result (κ −0.016 hand-set / −0.024 trained, composite ρ +0.468 vs IRT ρ +0.345, top-10 overlap 1/10; explanation grounded in `scripts/eval_label.py` — a draft that mis-described what the LLM rater sees was caught as a hallucination and corrected). §5.6.2 Tradeoffs (`\paragraph{Rater fidelity}` κ_lin +0.198 / κ_quad +0.262 / exact 38% / within-1 68% + `\paragraph{Model-versus-rater band agreement}` 47.3% / 71.7% / κ struggle +0.379 / κ difficulty +0.225). §5.6.3 Limitations (cohort skew + difficulty n=72 LOO + OpenAI scorer fallback midpoint as highest-leverage future change).
- **Housekeeping.** Stale `% TODO: 5.x.y prose block` comments swept — removed 17 across §5.1.1–§5.6.3 where prose now exists. Only the top-of-file `% TODO (Bakri): READ-THROUGH PASS` block remains, with a roadmap-sync reminder pointing at this note. Voice sweep is the user's read-through task before submission.
- **Outstanding for Ch5.** Appendix B Settings-view screenshot re-shoot (the deployed Settings view no longer has the removed weight-version card); Appendix E formulae are optional. The §5.5.5 triangulation paragraph references `\ref{sec:conclusion}` (Future Work) for the cross-session-aggregation feature request — will resolve once Ch6 lands.
- **Ch6 Conclusion** is being authored in a separate chat.
