...
Post-draft revision pass following the 2026-05-29 supervisor meeting, backed by two read-only audits of the whole report (verified against the `.tex` and `data/eval/` files — see `[[Meeting Log]]`). **Progress: Notes 01–05 are fully applied to `Report/`; Note 06 is closed (A1 OLS cite applied; A2 dropped); Notes 07–12 are pending (see the Status column).** The Note 05 pass also ran a deep code-vs-report accuracy audit that found and fixed a deployment bug (trained v2 weights were silently rejected — [[01 Integrity & Consistency Fixes]] I14) and several factual errors (I15–I18); a clean `latexmk` recompile confirmed this 2026-06-01 (exit 0, no undefined citations). Every `Report/`/`.bib` edit is previewed in these notes and applied only after sign-off. Prose is *briefed* here for the author to write; mechanical edits are previewed for approval.
...
| ✅      | [[05 Intros, Conclusions & Summaries]]                          | Chapter-intro audit, section closers, chapter summaries, abandoned sentences (+ re-audit gap-closure: model-name/router/constant fixes, V1/V2 redistribution, Fig 3.6) | author writes                  | done 2026-05-31                    |
| ✅      | [[06 Background Lit-Review Expansion]]                          | **Done 2026-06-01:** A1 OLS cite applied (§2.2.4); A2 help-seeking dropped (grounded by survey §5.5); eval refs → 07; B2 parked, B3/B5 dropped                         | author writes                  | done 2026-06-01 (latexmk clean)    |
