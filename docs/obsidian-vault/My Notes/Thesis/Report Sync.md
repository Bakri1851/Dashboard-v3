...
- §3.6.2 caveat "conceptual design rather than a fully implemented system" still in place; design-vs-implementation reframe deferred. Mockups themselves stay as design artifacts (correctly placed in Ch3).
...
- §3.5 Retrieval-Augmented Generation Feedback Design empty stub filled. Hybrid two-layer retrieval design (structured pre-filter then semantic search) presented at design-register only: generic concept names (vector store, embedding model, ANN index, language model) — no library / model / parameter specifics, which were deferred to Ch4 implementation per the user's design-vs-implementation refinement. Custom TikZ figure (`fig: rag architecture`) shows the pipeline flow with a dashed group around the two retrieval layers and a side-fed struggle-context branch. Two surfaces (assistant-side, instructor-side) framed as one shared pipeline with surface-specific outputs. Cited `lewisRetrievalAugmentedGenerationKnowledgeIntensive2020`, `kasneciChatGPTGoodOpportunities2023`, `reimersSentenceBERTSentenceEmbeddings2019`, `malkovEfficientRobustApproximate2016`. Dr Batmaz attribution removed from body per new memory rule [[feedback-supervisor-attribution]]; supervisor credit deferred to Acknowledgments.
...
- §3.6 typo pass cleaned: stray backtick in `dec\`ision-making` (line 776), `is will allow` (line 788), `based off` in Fig 8 caption (line 793), `have been help` in Fig 10 caption (line 810). **Figma-replacement reframe**: the mockups in §3.6.2 stay in Ch3 as design artifacts — the earlier Roadmap / Rewrite Queue framing about replacing them with V2 dashboard screenshots was incorrect, conflating design intent with implementation result. Deployed-system screenshots belong in Ch4 (Implementation) and Appendix B (UI Screenshots). The `conceptual design rather than a fully implemented system` caveat in §3.6.2 still needs softening to reposition the mockups as design specification rather than rough concept; that rewrite is deferred to a chapter-wide design-vs-implementation reframe pass after Step 3. New memory rule [[feedback-design-artifacts-in-design-chapter]] captures the principle.
- §3.6.3 Lab Assistant View empty stub filled. Three states (session join, waiting, assigned) described in design-register only; no library/parameter specifics. Three mockup figure slots consolidated into one `egin{figure}[H]` with three `egin{subfigure}[t]{0.31\linewidth}` side-by-side using the `subcaption` package (loaded at `main.tex:9`). Single shared caption with `\subref{}` cross-references; halves the page footprint compared with three full-row figures. Prose unified into four paragraphs (intro + one per state). Cross-references resolve to §3.5 (`sec: rag`) for the coaching prompts. Three PNGs (`figma4.png`, `figma5.png`, `figma6.png`) outstanding — user generating with Figma AI; figure block references the filenames so it populates on next compile.
- **Step 3 of [[Full Roadmap]] is now fully closed at the prose level.** The remaining design-vs-implementation reframe (§3.6.2 caveat softening) is a deferred polish pass and not blocking Step 4.
