...
See also [[Assistant App/Operations/Known Issues]] for the assistant data scope mismatch issue.

Related: [[Instructor Dashboard]], [[Analytics Engine]], [[Next Steps]]
...
- Session lifecycle sounds are miswired: `play_session_start()` and `play_session_end()` exist, but the instructor app currently calls `play_selection()` when session activity toggles. See [[UI System]] and [[Lab App/Flows/Pages and UI Flow]].

- ~~BKT and IRT leaderboard scores cluster near 1.0~~ **Resolved (Phase 4):** the improved struggle model now uses BKT mastery as one of three signal groups (mastery gap, weight 0.30) rather than the sole struggle metric. The mastery gap has naturally higher variance because it measures the difference between longitudinal mastery and current-session performance. See [[Improved Struggle Logic]] and [[BKT Mastery Logic]].
